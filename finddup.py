...
argc = len(sys.argv)
if argc <= 1:
    print "Usage: \n\t%s <dir[,dir[,dir[,...]]] [options]" % sys.argv[0]
    print "\t    options:"
...
for base_dir in search_path:
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            ...
            if wildcard_fname != None and not fnmatch.fnmatch(file, wildcard_fname): # file is not in wildcard pattern
                continue
            if reobj_fname != None and reobj_fname.match(file) == None: # file is not in regex pattern
                continue
            if not file_list.has_key(file):
                file_list[file] = {}
            AddFileProp(file_list[file], root, file)
        #end for in files
    # end for in os.walk
